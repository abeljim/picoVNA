...
class NanoVNA:
    ...
    def scan(self):
        with grpc.insecure_channel('localhost:50051') as channel:
            ...
            response = stub.RequestScan(pico_vna_pb2.ScanRequest())
            self._frequencies = response.freqs
            return ([data.real + 1.j * data.im for data in response.s11_data], [data.real + 1.j * data.im for data in response.s21_data])
        # segment_length = 101
        # array0 = []
        # array1 = []
    ...
    while True:
        if plot or opt.save:
            ...
            if opt.scan or opt.points > 101:
                ...
                if first:
                    pl.grid(True)
                    pl.xlim(nv.frequencies[0], nv.frequencies[-1])
                    first = False
            else:
